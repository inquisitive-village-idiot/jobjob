...
- [ ] 1.1 `webapp/frontend/src/pages/ApplicationsPage.tsx`: derive per-row
...
- [ ] 2.1 Built rows: render a small **Building…** badge NEXT TO the status
...
- [ ] 2.2 State filter: include a **BUILDING** chip in `presentStates` when any
...
- [ ] 3.1 Add a 5s poll of `fetchRuns` (mirror `QueuePage`'s `POLL_MS = 5000` +
...
- [ ] 4.1 Frontend: the repo has NO JS test runner — do NOT add one (out of
...
- [ ] 4.2 Docs: note the live BUILDING state on the Applications page in the
  relevant docs/ page.
