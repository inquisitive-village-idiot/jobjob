...
- [x] 1.1 `jobjob/autofill/runner.py`: add a wait mode that blocks on the human
...
- [x] 1.2 `jobjob/autofill/__main__.py`: select the window-close wait
...
- [x] 2.1 `webapp/backend/routers/jobs.py`: add `POST /jobs/apply` (autofill).
...
- [x] 2.2 Launch `python -m jobjob apply <url>` as a **detached** subprocess
...
- [x] 3.1 `webapp/backend/services/tracking_service.py`: add `posting_url` (the
  source `web_uri`, or None) to the completed-application item.

## 4. Frontend

- [x] 4.1 `webapp/frontend/src/pages/ApplicationsPage.tsx`: enable the reserved
...
- [x] 5.1 Backend: URL gating (400 without `web_uri`); launch wiring records a
  ...
  no real Playwright).
- [x] 5.2 Frontend: `tsc && vite build` + eslint green; Apply enable/disable by
  `posting_url`.
- [x] 5.3 Docs: note the webapp Apply action (autofill from the Applications
  table, gated on a posting URL) in CLAUDE.md / the relevant docs page.
- [x] 5.4 Run backend `pytest` as its own command; confirm green before push.
