...
- [x] 1.1 New `webapp/backend/services/application_source.py` (sibling of
...
- [x] 2.1 `application_metadata.py`: add `ensure_entity_id(folder) -> str` —
...
- [x] 2.2 `jobjob/apply/workflow.py` `run_application_workflow`: near
...
- [x] 3.1 `webapp/backend/services/run_history.py`: `finish_run(...)` accepts an
...
- [x] 3.2 `webapp/backend/routers/jobs.py`: after the build run returns, pass
  `results.get("entity_id")` into `finish_run` for build runs.

## 4. Id-preferring joins (name fallback)

- [x] 4.1 `webapp/backend/services/tracking_service.py`: surface `entity_id` on
  ...
  as today.
- [x] 4.2 `webapp/backend/routers/jobs.py` `_find_rerun_jd`: when the target
...
- [x] 5.1 `webapp/backend/routers/tracking.py`: add `PATCH
...
- [x] 5.2 Expose `entity_id` + source fields on the completed-application item
  so the frontend can render/edit them.

## 6. Contacts identity

- [x] 6.1 `jobjob/enrich/workflow.py` `write_profile_sidecar` (and the enrich
  flow): mint/reuse an `entity_id` and record it in the profile `.json` sidecar.
- [x] 6.2 `jobjob/gapi/sheets.py` `append_profile`: write the `entity_id` under
...
- [x] 7.1 `webapp/frontend/src`: show the short `entity_id` prefix as a small
...
- [x] 8.1 Backend tests: entity id mint-once + reuse-on-rebuild; `source.json`
...
- [x] 8.2 Frontend: `tsc && vite build` + eslint green; update tests referencing
  changed types/items.
- [x] 8.3 Docs: note the entity/source model (entity_id in metadata.json,
  source.json, editable fields) in CLAUDE.md and the relevant docs/ page.
- [x] 8.4 Run backend `pytest` as its own command; confirm green before push.
