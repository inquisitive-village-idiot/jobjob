...
- [ ] 3.1 `application_metadata.py`: rename enum `GENERATED` → `BUILT`
  (`value="BUILT"`); `DEFAULT_STATUS = ApplicationStatus.BUILT`
- [ ] 3.2 Add schema versioning: `_SCHEMA_VERSION = 2`; a `_migrate(data)` that,
...
- [ ] 3.3 `tracking_service.py`: add `GENERATED` as a legacy alias in
...
- [ ] 4.1 `run_history.py`: document-generation runs record `kind: "build"`;
...
- [ ] 8.1 Backend tests: CLI dispatch (`build`→generation, `apply`→autofill,
  ...
  stamped
- [ ] 8.2 Frontend: type-check/build green; update any tests referencing the old
  routes/kinds/GENERATED
- [ ] 8.3 Run backend suite (`pytest`) and the frontend build/type-check; confirm
  green
