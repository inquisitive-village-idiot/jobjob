...
class LinkedInProfile:
    ...
    NOTE: field docs drive the extraction prompt (see static/prompt/linkedin_profile.txt),
        the same way JobDescription does.
    """
    ...
    location: str = dcs.field(metadata={"doc": "Their location (city, region, country)"})
    headline: str = dcs.field(metadata={"doc": "Their LinkedIn headline / tagline"})
    linkedin_url: str = dcs.field(
        metadata={
            "doc": "Their LinkedIn profile URL (linkedin.com/in/...) if visible, else empty"
        }
    )
